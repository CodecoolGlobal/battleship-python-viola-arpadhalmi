...
def get_coordinates_ship(board, mode=1):
...
def validate_and_place_vertical_ships(board, row, col, mode=1):
    grid = 5
    if mode == 1:
...
def validate_and_place_horizontal_ships(board, row, col,mode =1):
    grid = 5
    if mode == 1:  
...
def place_one_block(board, mode=1):
    ...
    # if it's in corner
        ...
        return validate_and_place_corner_ships(validate_00_corner_ships_area(board), board, row, col, mode)
    elif row == 0 and col == 4:
        return validate_and_place_corner_ships(validate_04_corner_ships_area(board), board, row, col, mode)
    elif row == 4 and col == 0:
        return validate_and_place_corner_ships(validate_40_corner_ships_area(board), board, row, col, mode)
    elif row == 4 and col == 4:
        return validate_and_place_corner_ships(validate_44_corner_ships_area(board), board, row, col, mode)
    # edges
...
def place_two_block(board, mode =1):
    if mode == 1:
        ...
        row, col = get_coordinates_ship(board, 1)
    else:
...
def placing_ships(player_board, ships_1, ships_2, mode):
...
def main_placing_ships(mode=1):
    ...
    if mode == 1:
        print(f'\nPLAYER 1 MOVES\n')
        player_1_board = placing_ships(player1_b, player_1_one_block, player_1_two_block, mode)
        os.system(command)
        ...
        print(f'\nPLAYER 2 MOVES\n')
        player_2_board = placing_ships(player2_b, player_2_one_block, player_2_two_block, mode)
        os.system(command)
    else:
        player_2_board = placing_ships(player2_b, player_2_one_block, player_2_two_block, mode)
        os.system(command)
        print(f'\nPLAYER 1 MOVES\n')
        player_1_board = placing_ships(player1_b, player_1_one_block, player_1_two_block, 1)
...
def lets_sink(bullet, mode=1):
    ...
    if mode == 1:
        ...
        main_placing_ships(mode)
        print(f'{bcolors.BOLD}\nRemaining turns: {bullet}{bcolors.ENDC}\n')
        player = True
        print(f'\n   Player1          AI\n')

        for left, right in zip(print_board_yield(player1_b), print_board_yield(player2_b)):
            print(f'{left}    {right}')
        ...
        while bullet > 0:
            if player:
                ...
                player_turn(player1_b, player1_hidden_b, 2)
                sleep(2)
                os.system(command)
